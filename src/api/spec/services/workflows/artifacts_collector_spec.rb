...
RSpec.describe Workflows::ArtifactsCollector, type: :service do
  ...
  describe '#call' do
    ...
    context 'for a rebuild_package step' do
      ...
      let(:artifacts) do
        {
          project: step_instructions[:project],
          package: step_instructions[:package]
        }
      end
      ...
      let(:artifacts) do
        {
          project: step_instructions[:project],
          repositories: [
            { name: 'openSUSE_Tumbleweed', architectures: ['x86_64', 'ppc'] },
            { name: 'openSUSE_Leap_15.3', architectures: ['x86_64'] }
          ]
        }
      end
