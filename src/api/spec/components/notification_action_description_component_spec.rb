  ...
  context 'when the notification is for an Event::CreateReport' do
    context 'with the recipient being a user' do
      ...
      it 'renders a div containing who created a report and for what' do
        expect(rendered_content).to have_selector('div.smart-overflow', text: "User '#{notification.notifiable.reportable.user.login}' created a report for a comment for the following reason:Because reasons.")
      end
    end
  end
end
